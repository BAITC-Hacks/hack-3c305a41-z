...
Не реализованы и не имитируются нулями или фиктивными ответами: временные паттерны
из `transactions.parquet`, HITS, оценка устойчивости сети при изъятии узлов.
...
2. **Запустить расчёт:** `.\.venv\Scripts\python.exe run.py --data data --out out`
   В конце печатается время. Ограничение ТЗ — 300 с, фактически около 3 с.
...
целостность артефактов, отсутствие CDN, сценарии Streamlit, а также слой агентов: деградация без ключа, побайтовое совпадение выгрузок с флагом --llm и без него, обязательный проход через критика формулировок. Реальные три CSV повторного
...
Измерения: Windows, Python 3.13, первый прогон 3,03 с, повторный 2,87 с при лимите 300 с.
